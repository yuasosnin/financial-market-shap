    ...
    def __init__(self, input_size, seq_length, output_size=1, steps=1, 
        ...
        if self.fc_layers is not None:
            self.fc = nn.Sequential(
                nn.Linear(in_features=(self.bidirectional+1)*self.hidden_size*self.seq_length, out_features=self.fc_hidden_size),
                self.drop,
                self.act,
...
class CNNLSTM(nn.Module):
    def __init__(
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=self.cnn_channels[0], kernel_size=self.kernel_size[0]),
            nn.ReLU(),
            nn.Conv1d(in_channels=self.cnn_channels[0], out_channels=self.cnn_channels[1], kernel_size=self.kernel_size[1]),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2)
        )
        seq_length_out = l_out(l_out(l_out(self.seq_length, kernel_size=1, stride=1), kernel_size=2, stride=1), 2)
        self.lstm = nn.LSTM(
            bidirectional=self.bidirectional,
        ...
        if self.fc_layers is not None:
            self.fc = nn.Sequential(
                nn.Linear(in_features=(self.bidirectional+1)*self.hidden_size*seq_length_out, out_features=self.fc_hidden_size),
                self.drop,
                self.act,
                *[self.fc_bloc]*self.fc_layers,
                nn.Linear(in_features=self.fc_hidden_size, out_features=self.steps*self.output_size)
            ...
            self.fc = nn.Linear(in_features=(self.bidirectional+1)*self.hidden_size*seq_length_out, out_features=self.steps*self.output_size)
        self.init_weights()
...
class CNN(nn.Module):
    def __init__(self, input_size, seq_length, out_channels, droprate=0):
        super().__init__()
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=out_channels[0], kernel_size=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=out_channels[0], out_channels=out_channels[1], kernel_size=3),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),
            nn.Conv1d(in_channels=out_channels[1], out_channels=out_channels[2], kernel_size=3),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2),
        )
        self.fc = nn.Sequential(
            nn.Linear(in_features=int((int((seq_length-2)/2)-2)/2)*out_channels[-1], out_features=1),
            nn.Dropout(droprate)
        )
        self.init_weights()
