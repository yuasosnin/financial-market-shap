...
def take_vars(data: pd.DataFrame, idx: Sequence[str], var: Sequence[str],
              add: Optional[Sequence[str]] = None) -> pd.DataFrame:
    '''
...
def l_out(l_in: int, kernel_size: int, padding: int = 0,
          dilation: int = 1, stride: Optional[int] = None) -> int:
    '''
...
def read_model_logs(
        model_name: str, split: Literal['val', 'test'] = 'val') -> pd.DataFrame:
    '''
    ...
    for p in range(n_splits):
        for v in range(n_versions):
            y_true, y_pred = pd.read_csv(
                f'logs/model_{model_name}/period_{p}/version_{v}/results.csv').values.T
            errors_table['period'].append(p)
            errors_table['version'].append(v)
            errors_table['val_mse'].append(
                mse(y_true[-10:-5:], y_pred[-10:-5:]))
            errors_table['val_zero_mse'].append(
                mse(y_true[-10:-5:], np.zeros_like(y_pred[-10:-5:])))
            errors_table['test_mse'].append(
