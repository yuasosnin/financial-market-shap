...
def plot(
    ...
    if threshold:
        ...
        if threshold_v > abs_data.min():
            ...
            data = np.append(data[abs_data >= threshold_v],
                             sum(data[abs_data < threshold_v]))
    ...
    # Plot and label
    ax.bar(range(0, len(trans.index)), 
           blank,
           width=0.5, 
           color=blank_color, 
           **bar_kwargs)
    ...
    # Make sure the plot doesn't accidentally focus only on the changes in the
    # data
    ...
    ax.set_ylim(
        plot_min - round(3.6 * plot_offset, 7), 
        plot_max + round(3.6 * plot_offset, 7))
    ...
    ax.axhline(0, color='black', 
               **(dict(linewidth=0.6, linestyle='dashed') | line_kwargs))
